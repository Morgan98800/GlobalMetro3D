...
def get_canonical_manifest() -> dict:
    canonical_path = ROOT / "cities" / "paris" / "data" / "model-assets-manifest.json"
    if not canonical_path.exists():
        canonical_path = ROOT / "data" / "model-assets-manifest.json"
    return json.loads(canonical_path.read_text(encoding="utf-8"))


def test_train_asset_manifest_keeps_external_assets_gated() -> None:
    manifest = get_canonical_manifest()
