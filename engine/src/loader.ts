...
export interface ShapeData {
  ...
  ptCount: number;
  totalLengthM: number;
  // Flat Float32Array: [lng0, lat0, dist0, lng1, lat1, dist1, ...]
  points: Float32Array;
}
...
export interface TripStop {
  seq: number;
  stopId: string;
  arrTime: number; // seconds since midnight
  depTime: number;
  shapeDistM: number;
}
...
export class NetworkDataLoader {
  public shapes: Map<string, ShapeData> = new Map();
  public db: DatabaseSync;
  private tripStopsCache: Map<string, TripStop[]> = new Map();
  private stmtActiveTrips: any;
  private stmtTripStops: any;
  ...
  public getActiveServiceIds(dateStr: string, dayOfWeek: number): string[] {
    ...
    const calQuery = this.db.prepare(`
      ...
      WHERE ${dayCol} = 1
        ...
        AND end_date >= ?
    `);
    const calRows: any = calQuery.all(dateStr, dateStr);
    const activeSet = new Set<string>(calRows.map((r: any) => r.service_id));
    ...
    const excQuery = this.db.prepare(`
      ...
      WHERE date = ?
    `);
    const excRows: any = excQuery.all(dateStr);
  ...
  public getActiveTripsAtTime(timeSeconds: number, activeServiceIds: string[]): ActiveTrip[] {
    const tripsRows: any = this.stmtActiveTrips.all(
    ...
    for (const r of tripsRows) {
      ...
      if (!stops) {
        const stopRows: any = this.stmtTripStops.all(tid);
