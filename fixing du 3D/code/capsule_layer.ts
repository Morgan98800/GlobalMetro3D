...
export function createCapsuleLayers(params: CapsuleLayerParams): any[] {
  ...
  const profileMeasure = typeof window !== 'undefined' ? (window as any).__profileMeasure : null;
  const measured = <T>(name: string, callback: () => T, items = 1): T => {
    if (!profileMeasure) return callback();
    const startedAt = performance.now();
    const result = callback();
    profileMeasure(name, performance.now() - startedAt, items);
    return result;
  };
  const measuredSlice = (shape: ShapeEntry, start: number, end: number) =>
    measured('capsule.sliceShape', () => sliceShape(shape, start, end));
  ...
  for (const train of activeTrains) {
    ...
    const debugSlice = debugShape ? measuredSlice(debugShape, tailD, headD) : [];
    if (debugTrains) {
    ...
    const fullPath = measuredSlice(shape, tailD, headD);
    ...
    const bodyColor = measured('capsule.getNeutralBodyColor', () => getNeutralBodyColor(train.colorHex));
    ...
    if (!isDetailed) {
      ...
      // -----------------------------------------------------------------------
      const cars = measured(
        'capsule.splitIntoCars',
        () => splitIntoCars(shape, headD, stock.cars_count, stock.car_length_m, stock.inter_car_gap_m, 1),
        stock.cars_count
      );
      ...
      for (let i = 0; i < stock.cars_count - 1; i++) {
        ...
        const gangwayPath = measuredSlice(shape, Math.max(0, nextCarHeadD), Math.max(0, carTailD));
      ...
      const nosePath = measuredSlice(shape, noseTailD, headD);
      ...
      const lights = measured('capsule.computeTrainLights', () => computeTrainLights(shape, headD, tailD, stock.width_m, elev));
